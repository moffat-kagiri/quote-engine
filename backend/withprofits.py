"""
WithProfit Policy Projection Engine

Handles accumulation projections for unit-linked with-profit policies including:
- Initial deposits and top-ups
- Scheduled withdrawals
- Running balance calculations with guaranteed returns
- Death and PTD benefits calculated on accumulated balance at each year
"""
...
class WithProfitProjector:
    """
    Projects with-profit policy accumulation over time.
    ...
    - Remaining balance grows at guaranteed return rate
    - Death/PTD benefit = accumulated balance (no fixed minimum)
    - Rate selection: <5 years use 5-year rate, >20 years use 20-year rate
    """

    def __init__(self, 
                 death_rate_by_term: Dict[int, float],
                 ptd_rate_by_term: Dict[int, float],
                 guaranteed_return_rate: float):
        """
        ...
        Args:
            ...
            guaranteed_return_rate: Annual guaranteed return rate (e.g., 0.09 for 9%)
        """
        self.death_rate_by_term = death_rate_by_term
        self.ptd_rate_by_term = ptd_rate_by_term
        self.guaranteed_return_rate = guaranteed_return_rate
    ...
    def project(self,
        ...
        """
        Project policy accumulation over term.
        ...
        for year in range(1, term_years + 1):
            ...
            # Apply guaranteed return to remaining balance
            fund_growth = balance_after_costs * self.guaranteed_return_rate
        ...
        return {
            ...
            "summary": {
                ...
                "guaranteed_return_rate": self.guaranteed_return_rate,
            }
        }
...
def calculate_withprofit_premium(age: int,
                                  sa: float,
                                  term: int,
                                  death_rate: float,
                                  ptd_rate: float) -> float:
    """
    Calculate monthly premium for with-profit product.
    
    For with-profit, the premium is based on:
    - Cost of death benefit (fixed 100,000)
    - Cost of PTD benefit (fixed 100,000)
    - Allocable portion for fund accumulation
    
    Args:
        age: Policyholder age
        sa: Sum assured requested (used for fund target)
        term: Policy term in years
        death_rate: Annual death premium rate
        ptd_rate: Annual PTD premium rate
    
    Returns:
        Monthly premium in KES
    """
    # Base calculation: use SA or minimum 200000 for fund target
    base_amount = max(200000, sa)
    
    # Fixed cost of death and PTD benefits on 100,000
    fixed_death_cost = 100000 * death_rate
    fixed_ptd_cost = 100000 * ptd_rate
    
    # Allocable portion grows fund (simplified: linear allocation)
    fund_accumulation_monthly = (base_amount / (term * 12)) * 0.95  # 5% loading for costs/profits
    
    # Total monthly premium
    total_monthly = (fixed_death_cost + fixed_ptd_cost) / 12 + fund_accumulation_monthly
    
    return round(total_monthly)
